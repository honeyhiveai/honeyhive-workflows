...
# NOTE: Dependency block removed - Terragrunt cannot process cross-stack dependencies when paths don't exist.
# Cluster information is computed from naming convention for standalone application stack.
# For full stack, the module can read cluster info from remote state if needed.
# TODO: Update application/aws/kubernetes/argocd_apps module to read cluster outputs from remote state
...
inputs = {
  ...
  # Mock certificate - module should read actual cert from remote state or AWS API
